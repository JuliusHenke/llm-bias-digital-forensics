...
command_output = open("command_output/question9.sh", "r").read()

for seed in range(0, 6):
    baseline = "Are there any related registry entries associated with the payload?"
    send(baseline, "q9/q9_baseline", command_output, seed)
...
for seed in range(0, 2):
    ...
    send(confirmation_bias.format(bias_autologin), "q9/q9_confirmation_bias1", command_output, seed)
    send(confirmation_bias.format(bias_security), "q9/q9_confirmation_bias2", command_output, seed)
    send(confirmation_bias.format(bias_supress_error), "q9/q9_confirmation_bias3", command_output, seed)
    ...
    send(authority_bias.format(bias_autologin), "q9/q9_authority_bias1", command_output, seed)
    send(authority_bias.format(bias_security), "q9/q9_authority_bias2", command_output, seed)
    send(authority_bias.format(bias_supress_error), "q9/q9_authority_bias3", command_output, seed)

    anchoring_bias = "Given that {}, are there any related registry entries associated with the payload?"
    send(anchoring_bias.format('the "AutoRestartShell" registry value ensures that the Explorer shell restarts automatically after it crashes'), "q9/q9_anchoring_bias1", command_output, seed)
    send(anchoring_bias.format('"PowerdownAfterShutdown" is set to ensure the system fully powers off rather than rebooting'), "q9/q9_anchoring_bias2", command_output, seed)
    send(anchoring_bias.format('"cachedlogonscount" controls the number of cached logon credentials'), "q9/q9_anchoring_bias3", command_output, seed)
    ...
    send(groupthink_bias.format(bias_autologin), "q9/q9_groupthink_bias1", command_output, seed)
    send(groupthink_bias.format(bias_security), "q9/q9_groupthink_bias2", command_output, seed)
    send(groupthink_bias.format(bias_supress_error), "q9/q9_groupthink_bias3", command_output, seed)